 ...
 *  未覆盖的接口回退为原始路径。 */
const API_LABELS: Record<string, { n: string; d: string }> = {
  "/api/quotes": { n: "统一报价", d: "指数/个股实时报价(统一报价中心)" },
  "/api/minute": { n: "分时数据", d: "个股/指数日内分时走势" },
  "/api/boards": { n: "板块排行", d: "行业/概念板块涨跌排行" },
  "/api/board-stocks": { n: "板块成分股", d: "板块内成分股列表" },
  "/api/rank": { n: "个股榜单", d: "涨跌幅/成交额/换手率排行" },
  "/api/moneyflow": { n: "资金流", d: "个股资金流向排行" },
  "/api/stock-flow": { n: "个股资金流", d: "单只个股资金流向" },
  "/api/stock-flows": { n: "个股资金流(批量)", d: "批量获取多只个股资金流" },
  "/api/stock-main-forces": { n: "个股主力净额", d: "个股主力净流入/主动买卖" },
  "/api/board-flow": { n: "板块资金流", d: "板块累计主力净流入曲线" },
  "/api/stock-boards": { n: "个股板块", d: "个股所属行业/地域/概念" },
  "/api/stock-profile": { n: "个股主营业务", d: "个股主营业务简介" },
  "/api/stock-quote": { n: "个股盘口", d: "个股实时盘口行情" },
  "/api/stock-detail": { n: "个股详情聚合", d: "个股详情(本地库+按需抓取+失败回退)" },
  "/api/stock-finance": { n: "个股财务", d: "个股财务指标摘要" },
  "/api/news": { n: "快讯", d: "实时财经快讯" },
  "/api/treasuries": { n: "美债收益率", d: "美债即时收益率" },
  "/api/treasury-history": { n: "美债历史曲线", d: "美债月度收益率历史曲线" },
  "/api/finance-main": { n: "财报主指标", d: "单公司近12期财务主指标" },
  "/api/finance-board": { n: "财报榜单", d: "盈利榜+行业聚合+披露日历" },
  "/api/finance-forecast": { n: "业绩预告", d: "财报业绩预告统计" },
  "/api/health": { n: "健康检查", d: "服务存活与缓存状态" },
  "/api/monitor": { n: "系统监控", d: "接口性能/DB状态/内存监控" },
  "/api/openrouter-usage": { n: "用量统计", d: "OpenRouter 模型用量统计" },
  "/api/stock-search": { n: "股票搜索", d: "名称/拼音首字母搜索股票" },
  "/api/plugin-news-analyst": { n: "新闻分析师", d: "新闻/社交/财务情绪分析" },
  "/api/plugin-market-sentiment": { n: "市场情绪", d: "市场情绪指标与涨跌统计" },
  "/api/fengk-front": { n: "风口榜", d: "市场板块实时热点(风口榜)" },
};

/** 取接口中文名称; 未收录时回退为原始路径 */
function apiLabel(path: string) {
  return API_LABELS[path] ?? { n: path, d: "" };
}
        ...
        <div className="rounded border border-[#e0d5c0] bg-[#f5f0e6]/60 p-2">
          ...
          <table className="w-full text-[10px]">
            ...
            <tbody>
              ...
              {sorted.length === 0 && (
                <tr><td colSpan={7} className="py-3 text-center text-[#a8987e]">暂无接口数据</td></tr>
              )}
            </tbody>
          </table>
        </div>
